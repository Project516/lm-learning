# print("hello people")

# print("Blue and gold,\nThe team stands strong and bold,\nGo team, fight!")

# print("  @..@\n (----)\n(>____<)\n^^~~~~^^")

# student_name = "Alex"

# print(student_name + " was a dedicated computer science student at the university.")
# print("Each morning " + student_name + " biked through the vibrant campus.")
# print(student_name + " enjoyed all of the interesting CS classes.")
# print("During the week, "+ student_name + " would meet up with friends to collaborate on challenging programming projects.")
# print("On weekends, " +student_name+ " participated in hackathons.")
# print("Computer science allowed "+student_name+ " to think logically and creatively.")

# name = "Sam"
# age = 16
# print(name)
# print(age)

# age = 17
# favorite_color = "green"

# print(age)
# print(favorite_color + "\nMy favorites are ", age , " " +favorite_color)
